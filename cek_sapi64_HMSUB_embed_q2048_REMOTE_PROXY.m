...
% to generate Jij file, run python script "TEST_genIsingCoeffs_subH.py" in
%  ...\__Q_2018\__DWave\DWAVE_PY_ISING
M=12;
N=3;
Jij_DIR='../../Jij_DATA/';
fname=strcat(Jij_DIR, strcat( strcat(strcat(strcat('Jij_sub_H',int2str(M)),'_'), ...
    int2str(N)), '.txt'));
%
% Number of qubits
NQ=M*N+ M*round(N*(N-1)/2);
...
myProxy='http://proxy_usr:proxy_passwd@your.proxy.address:port';
    ...
    solver = sapiSolver(remoteConn, 'DW_2000Q_VFYC_2_1'); %DW_2000Q_VFYC_2 %DW_2000Q_2
...
%
probError_HMSUB(aconf,newAnswer,M, N);
