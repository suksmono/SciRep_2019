...
-------------------------------------------------------------------------------
>> observation of scaling effect::
    >> increase iteration, obserserve probability of success
"""
from sympy import *
...
##
def isHadamard(G):
    #check whether G an orthogonal matrix or not
    # calculate indicator matrix
    D=np.matmul(G.tolist(), G.transpose().tolist() )
    M=len(D)
    ttl=0
    for m in range(0,M):
...
'''
VEC_ITR=[1000,5000,10000,50000,100000,500000]
for k in range(len(VEC_ITR)):
    print('\nSweep Number ->',VEC_ITR[k])
    ...
    NSWEEPS=VEC_ITR[k] #1000 #1*5*10*10*1000
    response = solver.sample_ising(h, J, sweeps=NSWEEPS, num_reads=100)
    #
    ...
    vSol=tSol #[0]
    #
    HM=vSol[0:M*M].reshape(M,M)
    #print('Found matrix:\n',HM)
    #print('Indikator matrix:\n', \
    #       np.matmul(HM.tolist(), HM.transpose().tolist() ))
    
    ## count the number of correct solution
    if isHadamard(HM)==1:
        minE=vE[idxMinE]
        minE=int(minE*10000)/10000
        cntCorrect=0
        for m in range(len(vE)):
            if not(vE[m]>minE):
                #print('m=',m)
                cntCorrect=cntCorrect+1
        print('number of Correct solution:', cntCorrect)
    else:
        print('There is no correct solution')
